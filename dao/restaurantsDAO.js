...
export default class RestaurantsDAO {
    ...
    static async getRestaurantById(id) {
        try {
            const pipeline = [
                // return restaurant that matches given `id`
                {
                    $match: {
                        _id: new ObjectId(id),
                    }
                },
                // also, return reviews on this restaurant
                {
                    $lookup: {
                        from: "reviews",
                        let: {
                        ...
                        pipeline: [
                            // return review whose restaurant id matches with this restaurant
                            {
                                $match: {
                                    $expr: {
                                        $eq: ["$restaurant_id", "$$id"],
            ...
            console.error(`Something went wrong in getRestaurantsById: ${err}`)
            throw err
        }
    }
